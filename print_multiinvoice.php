...
@media screen and (max-width: 600px) {
  ...
  <body>
    <?php 
     ...
     // $transaction_id = $_POST['transaction_id'].'<br>';
     $transaction_id="omvpoce";
...
<?=($fetchInvoice['invoice_quotation']=="quotation")?"QUOTATION":"PROFORMA INVOICE"?></p></center>
      ...
      <div class="row">
        ...
        <div class="row">
            <?php if (!empty($fetchInvoice['transaction_next_date'])): ?>
...
<div class="row mb-2" style="background-color: #D6EAF8;">
  ...
  <div class="col-sm-6 ">
    ...
    <div class="row" style="background-color: #21618C;color:white;">
      ...
      <?php if ($shipcount>0): ?>
        <tr><th>Currency</th><td><?=strtoupper($fetchInvoice['invoice_currency'])?></td></tr>
        <tr><th>Shipment Term</th><td><?=empty($fetchInvoice['invoice_fright'])?"FOB":"CNF"?></td></tr>
        <tr><th>Ship Via</th><td><?=strtoupper($shipper['shipment_type'])?></td></tr>
        <tr>
        <th>PORT OF LOADING</th><td><?=@strtoupper($shipper['shipment_port_of_landing'])?> JAPAN</td></tr>
        ...
        <tr><th>Final Destination</th><td>
           <?=@ucfirst(fetchRecord($dbc,"country_regulation","country_regulation_id",$fetchInvoice['invoice_country_port'])['country_regulation_destination_port'])?>
        </td></tr>
            <?php endif ?>
            <?php if ($shipcount<1): ?>
             <tr>
...
<!-- <div class="row" style="backgrou d-color: #21618C;color:white;">
   ...
   $fetchInvoice = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT invoice.*, transactions.*, customers.*, vehicle_info.*, brands.*, maker.* FROM invoice INNER JOIN transactions ON invoice.transaction_id = transactions.transaction_id LEFT OUTER JOIN customers ON customers.customer_id = invoice.invoice_customer LEFT OUTER JOIN vehicle_info ON vehicle_info.vehicle_id = transactions.vehicle_id LEFT OUTER JOIN brands ON brands.brand_id = vehicle_info.vehicle_brand  LEFT OUTER JOIN maker ON maker.maker_id = vehicle_info.vehicle_maker WHERE invoice.invoice_id = '$invoice_id'")); ?>
            <tr>
             ...
             <!--  <td><?=$fetchInvoice['vehicle_m3'] ?></td> -->
              <td rowspan="2"><?=$fetchInvoice['invoice_grand_total']?> <?=strtoupper($fetchInvoice['invoice_currency'])?></td>         
        ...
        ?>
          ...
          <?php 
$sub_total_amount+=$fetchInvoice['invoice_grand_total'];
        } ?>
          </tbody>
        </table>
      </div>
  ...
  <tfoot>
    ...
    <tr><td colspan="4" class="text-right" style="color: #21618C;font-weight: bold;">SUBTOTAL USD <?=strtoupper($fetchInvoice['invoice_currency'])?></td><td class="cd"><?=$sub_total_amount?></td></tr>
...
if ($invoiceFetch['gifted']==1) {
 ...
 ?>
    ...
    <tr>
      <td><?=$getServices['vehicle_services_id']?></td>
  ...
  <tfoot>
    <tr>
      <td colspan="3"></td>
       <td class="cd font-weight-bold" colspan="2">SERVICES TOTAL <?=@strtoupper($fetchInvoice['invoice_currency'])?></td><td><?=@$Subtotal_price?></td>
    </tr>
  </tfoot>
</table>
</div>
...
<div class="row"><div class="col-sm-12"><hr style="padding: 2px;width: 100%;background-color:#21618C " ></div></div>
   <div class="row">
    ...
    <tr><td colspan="2" class="b-1"><h6 class="font-weight-bold">BANKING INFORATION</h6></td>
     <!--  <td class="cd font-weight-bold">SERVICES TOTAL USD $</td><td><?=@$Subtotal_price?></td> --></tr>
    ...
    <tr><td class="b-1 h6">SWIFT CODE</td><td class="b-1"><?=$bankDetail['customer_contact_person']?></td></tr>
     ...
     <div class="col-sm-4">
       <table class="table table-borderedless text-center table-sm" style="color:black ">
         ...
         <tr> <td  class="cd font-weight-bold">SALE TAX RATE %</td><td>0.0</td></tr>
        <tr>  <td class="cd font-weight-bold">TOTAL UNIT </td><td><?=$count_units?></td></tr>
        <tr>  <td class="cd font-weight-bold">TOTAL PARTS </td><td><?=@$total_units?></td></tr>
        <tr>  <td class="cd font-weight-bold">SALES TAX <?=strtoupper($fetchInvoice['invoice_currency'])?></td><td>0.0</td></tr>
        <tr>  <td style="border-top: 3px solid black;" class="cd font-weight-bold">TOTAL BILL</td><td style="border-top: 3px solid black;" class="cd font-weight-bold"><?=$total_Amount?></td></tr>
       </table>
     </div>
   </div>
  <div class="row">
